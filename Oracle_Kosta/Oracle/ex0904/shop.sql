...
create table order_line( 
  order_line_id number(5) primary key, --주문상세 코드
  order_id number(5) not null references orders(order_id), --주문코드(주문id)
  goods_id varchar2(20) not null references goods(goods_id), --상품id
  unit_price number(12) not null, --단가(할인된 가격)
  qty number(3) not null, --주문 수량
  amount number(10) not null -- 주문수량 * 단가 한 가격
);
...
--주문하기
 ...
 3) JANG 아이디가 A03 상품 2개와 A04 상품 1개 구입한다.
  ...
  1) 
  2)위에서 검색된 각 레코드의 order_id에 해당하는 주문상세를 검색
  select * form order_line where order_is = 3;
