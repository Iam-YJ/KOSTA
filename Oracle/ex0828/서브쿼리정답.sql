...
where emp_name like '%정%' and sal> (select avg(sal) from subemp) and dept_id is not null;

select  *from subemp 
where dept_id in (select dept_id from subEMP
where emp_name like '%정%' and sal> (select avg(sal) from subemp) and dept_id is not null); --이게 정답임
