...
public class MainApp {
	...
	public static void main(String[] args) {
		...
		/**
		 * ApplicationContext : BeanFactory(객체 생성 + 소멸)의 확장개념(다국어 지원, 메세지 처리) : 객체를 사전
		 * 초기화(미리 메모리에 할당한다 <- scope = "singleton" : 만약, 지연초기화(필요할 때마다 새롭게 생성) <- scope
		 * = "prototype" 변경 이렇게 설정하면 getBean을 요청할 때마다 새로운 객체를 생성해준다.
		 */
		ApplicationContext context =
				// new ClassPathXmlApplicationContext("sample01/applicationContext.xml"); //진짜
				// 실행되는 파일 기준
