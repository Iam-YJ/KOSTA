...
/**
 * Board 영역의 등록, 전체검색, 상세보기, 수정, 삭제 등의 페이지를 실행하기 전에 세션 유무체크를 통해 세션이 있는 경우에만 페이지를
 * 실행하고 없으면 로그인 "인증하고 오세요" 메세지 출력
 *
 */
@Service
...
public class SessionCheckAdvice {
	...
	public void before(JoinPoint point) { //해당 기능을 담당하는 컨트롤러에 각각의 메소드 같은 위치에 HttpSession session를 생성한다
		...
		/*
		 * Object params[] = point.getArgs(); HttpSession session =
		 * (HttpSession)params[0]; //params의 [0]에 HttpSession session이 있기 때문에 이것만 필요하다
		 * 
