...
public class TransactionDAO {
	...
	public void transfer(String inputAccount, String outputAccount, int money) {
		...
		try {
			...
			if (result == 0) {
				throw new SQLException("출금하기에서 예외발생하여 계좌이체 실패했습니다.");
			}
			...
			if (result == 0) {
				throw new SQLException("입금하기에서 예외 발생하여 계좌이체 실패");
			}
			// 총액확인
			if (!balanceMaxCheck(con, inputAccount)) { // 금액초과
				throw new SQLException("입금계좌의 총액이 초과하여 계좌이체 실패 ");
			}
	 ...
	 * 입금계좌 총액 확인하기
	 * 
	 * @return : true이면 가능(commit 가능) , false 안됨 (rollback 처리해)
	 */
