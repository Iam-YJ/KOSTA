	...
	public MenuExam() {
		...
		save.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					// inputStream을 Reader에 넣어서 byte 읽기에서 한 줄 읽기로
					// 그 다음에는 buffer 사용해서 속도를 빠르게
				 ...
				 * {
				 * 
				 * bw.write(text.getText()); bw.flush();
				 * 
		...
		open.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (mode == JFileChooser.APPROVE_OPTION) {
					...
					try {
						...
						while ((br.readLine()) != null) {
							...
							text.append(str + "\n");
							// Reader를 사용해서 append를 하면.. 마지막에 "\n" 때문에 한줄이 더 들어감
							// + append를 쓰면 한번 지우고 다시 파일을 열어야된다는 단점
						}
				 ...
				 * FileInputStream(file))){ //파일에 있는 데이터를 읽기
				 * 
				 * byte [] b = new byte[bis.available()]; //읽을 수 있는 byte의 수 리턴 bis.read(b);
				 * 
				 * //textArea에 넣기 text.setText(new String(b, "UTF-8")); }catch(Exception errors)
				 * { e.printStackTrace(); }
				 */
			}
		});
