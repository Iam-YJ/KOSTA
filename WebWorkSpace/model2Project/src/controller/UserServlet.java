 ...
 */
// *.do는 UserServlet을 호출할 때 사용하는 url-pattern 이다.
@WebServlet("*.do")
public class UserServlet extends HttpServlet {
	...
	public UserServlet() {
		System.out.println("UserServlet 생성자 호출 ");
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		} else if (cmd.contentEquals("userJson")) {
			userJson(request, response);
		} else if (cmd.contentEquals("userJsonDelete")) {
			userJsonDelete(request, response);
		}
	}// doGet

	private void userJsonDelete(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String userid = request.getParameter("userid");
		int cnt = dao.deleteUser(userid);
		
		//정상적으로 삭제가 잘 되었으면 userJson method를 호출해서 Json 문자열을 클라이언트에게 다시 전달한다
		if(cnt==1) {
			userJson(request, response);
	...
	private void userJson(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		// DAO에서 받은 List<UserVO>를 JSON 데이터로 변환하고
